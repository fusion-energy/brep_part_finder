...
class TestShape(unittest.TestCase):
    ...
    def test_dict_keys_are_correct_type(self):
        ...
        for entry in self.brep_part_properties.keys():
            ...
            assert isinstance(self.brep_part_properties[entry]['BoundingBox.xmin'], float)
            assert isinstance(self.brep_part_properties[entry]['BoundingBox.ymin'], float)
            assert isinstance(self.brep_part_properties[entry]['BoundingBox.zmin'], float)
            assert isinstance(self.brep_part_properties[entry]['BoundingBox.xmax'], float)
            assert isinstance(self.brep_part_properties[entry]['BoundingBox.ymax'], float)
            assert isinstance(self.brep_part_properties[entry]['BoundingBox.zmax'], float)

    def test_volumes_are_positive(self):
        """Checks each Brep solid entry has a positive value for the volume"""

        for entry in self.brep_part_properties.keys():
            assert self.brep_part_properties[entry]['Volume'] > 0.
